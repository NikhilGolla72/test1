...
def _extract_plain_answer_text(value):
    if isinstance(value, dict):
        for key in ("explanation", "answer", "text", "message", "output_text"):
            candidate = value.get(key)
            if isinstance(candidate, str) and candidate.strip():
                extracted = _extract_plain_answer_text(candidate)
                if extracted:
                    return extracted
        for nested in value.values():
            extracted = _extract_plain_answer_text(nested)
            if extracted:
                return extracted
        return ""
...
def handler(payload, context):
    ...
    try:
        ...
        if direct_answer:
            ...
            result = agent(prompt)
            extracted_answer = _extract_plain_answer_text(result)
            answer = _sanitize_answer_text(extracted_answer or str(result))
            if not answer:
